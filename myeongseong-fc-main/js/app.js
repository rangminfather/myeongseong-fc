...
function getLatestUpdateVersion() {
  if (typeof UPDATE_HISTORY === "undefined" || !Array.isArray(UPDATE_HISTORY) || UPDATE_HISTORY.length === 0) {
    return APP_VERSION;
  }
  return UPDATE_HISTORY[0].version || APP_VERSION;
}
...
function renderUpdateNotice() {
  ...
  if (compareVersion(latest, APP_VERSION) > 0) {
    noticeEl.textContent = `새 업데이트(${latest})가 있습니다. 업데이트 확인 버튼을 눌러주세요.`;
...
function showUpdateHistory() {
  ...
  const text = UPDATE_HISTORY
    .map((item) => {
      const changes = Array.isArray(item.changes)
        ? item.changes.map((c) => `- ${c}`).join("\n")
        : "";

      return `[v${item.version}] ${item.date}
${item.title}
${changes}
코멘트: ${item.comment || "-"}`;
    })
...
async function checkForUpdate() {
  ...
  // 설치 중인 워커가 있으면 설치 완료까지 기다렸다가 적용
  if (reg.installing) {
